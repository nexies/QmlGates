...
Resizable::Resizable(QWidget *target) :
    QObject(target),
    _target(target),
...
bool Resizable::eventFilter(QObject *o, QEvent*e) {
...
void Resizable::mouseHover(QHoverEvent *e) {
    updateCursorShape(_target->mapToParent(e->pos()));
}

void Resizable::mouseLeave(QEvent *e) {
    if (!_leftButtonPressed) {
        _target->unsetCursor();
    }
}

void Resizable::mousePress(QMouseEvent *e) {
...
void Resizable::mouseRealese(QMouseEvent *e) {
    if (e->button() & Qt::LeftButton) {
        _leftButtonPressed = false;
        _dragStart = false;
    }
...
void Resizable::mouseMove(QMouseEvent *e) {
    if (_leftButtonPressed) {
        if (_dragStart) {
...
void Resizable::updateCursorShape(const QPoint &pos) {
...
void Resizable::calculateCursorPosition(const QPoint &pos, const QRect &framerect, Edges &_edge) {
